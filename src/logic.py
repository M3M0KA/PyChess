...
def is_rochade_possible(board, side, current_color, piece, x1, x2, y1, y2):
    global w_has_king_moved, w_has_left_rook_moved, w_has_right_rook_moved
    global b_has_king_moved, b_has_left_rook_moved, b_has_right_rook_moved
    if side == "right":
        if current_color == "W":
            if not w_has_king_moved and y1 == 7 and x1 == 4:
                if board[7][5] is None and board[7][6] is None:
                    if (x2 == 7 or x2 == 6) and not w_has_right_rook_moved:
                        copy_board = copy.deepcopy(board)
                        if not is_check(copy_board, "W", find_king(copy_board, "W")):
                            copy_board[7][5] = King("W")
                            copy_board[7][4] = None
                            if not is_check(
                                copy_board, "W", find_king(copy_board, "W")
                            ):
        ...
        if current_color == "B":
            if not b_has_king_moved and y1 == 0 and x1 == 4:
                if board[0][5] is None and board[0][6] is None:
                    if (x2 == 7 or x2 == 6) and not b_has_right_rook_moved:
                        copy_board = copy.deepcopy(board)
                        if not is_check(copy_board, "B", find_king(copy_board, "B")):
                            copy_board[0][5] = King("B")
                            copy_board[0][4] = None
                            if not is_check(
                                copy_board, "B", find_king(copy_board, "B")
                            ):
        ...
        if current_color == "W":
            if not w_has_king_moved and y1 == 7 and x1 == 4:
                if board[7][3] is None and board[7][2] is None and board[7][1] is None:
                    if (x2 == 0 or x2 == 1) and not w_has_left_rook_moved:
                        copy_board = copy.deepcopy(board)
                        if not is_check(copy_board, "W", find_king(copy_board, "W")):
        ...
        if current_color == "B":
            if not b_has_king_moved and y1 == 0 and x1 == 4:
                if board[0][3] is None and board[0][2] is None and board[0][1] is None:
                    if (x2 == 0 or x2 == 1) and not b_has_left_rook_moved:
                        copy_board = copy.deepcopy(board)
                        if not is_check(copy_board, "B", find_king(copy_board, "B")):
...
def move_piece(board, start, end, current_color, ai, gui):
    ...
    if current_color == "W" and isinstance(piece, King):
        if is_rochade_possible(board, "right", current_color, piece, x1, x2, y1, y2):
            board[7][6] = King("W")
            board[7][4] = None
            board[7][5] = Rook("W")
            board[7][7] = None
        ...
        if is_rochade_possible(board, "left", current_color, piece, x1, x2, y1, y2):
            if board[7][3] is None and board[7][2] is None and board[7][1] is None:
                board[7][2] = King("W")
                board[7][4] = None
                board[7][3] = Rook("W")
    ...
    if (
        piece
        and piece.color == current_color
        and piece.is_valid_move((x1, y1), (x2, y2), board)
    ):
        target = board[y2][x2]
        if target is None or target.color != current_color:
            ...
            if (
                is_check(temp_board, "W", find_king(temp_board, "W"))
                and current_color == "W"
            ):
                print("Weisser König im Schach!")
                return False
            if (
                is_check(temp_board, "B", find_king(temp_board, "B"))
                and current_color == "B"
            ):
            ...
            board[y1][x1] = None
            if (
                is_check(board, "W", find_king(board, "W"))
                and current_color == "B"
                and is_checkmate(board, "W")
            ):
                print("Schwarz hat gewonnen!")
                gui.win("Schwarz")
            if (
                is_check(board, "B", find_king(board, "B"))
                and current_color == "W"
                and is_checkmate(board, "B")
            ):
            ...
            if current_color == "W":
                ...
                stalemate_color = "W"
            if is_stalemate(board, stalemate_color):
                gui.win("stalemate")
            return True
    return False
...
def is_stalemate(board, color):
    if is_check(board, color, find_king(board, color)):
        return False
    for y in range(8):
        for x in range(8):
            piece = board[y][x]
                ...
                if piece.color == color:
                    for tx in range(8):
                        for ty in range(8):
                            ...
                            if piece.is_valid_move((x, y), (tx, ty), board):
                                board_copy[y][x] = None
                                board_copy[ty][tx] = piece
                                if not is_check(
                                    board_copy, color, find_king(board_copy, color)
                                ):
                                    return False
                            if is_rochade_possible(
                                board, "right", color, piece, x, y, tx, ty
                            ) or is_rochade_possible(
                                board, "left", color, piece, x, y, tx, ty
                            ):
                                return False
    return True
                    ...
                    for temp_x in range(8):
                        if piece.is_valid_move((x, y), (temp_x, temp_y), board):
                            ...
                            if not is_check(
                                temp_board,
                                king_color,
                                find_king(temp_board, king_color),
                            ):
